...
function parseFractionToDecimal(quantityStr: string): number {
  ...
  if (quantityStr.includes("-")) {
    const parts = quantityStr.split("-").map((p) => parseFractionToDecimal(p.trim()))
...
function parseEquipmentFromMarkdown(markdown: string): string[] {
  ...
  if (equipMatch) {
    ...
    const items = equipText.split(/\s+/).filter((item) => item.length > 3 && !item.match(/^(and|or|the|a|an)$/i))
...
if (import.meta.main) {
  ...
  results.forEach((r, i) => {
    console.log(
      `Recipe ${i + 1}: ${r.recipe.title} - ${(r.confidenceScore * 100).toFixed(1)}%`,
    )
